...
Snapshot::Snapshot(PayloadType pt, TagsMatcher tm, lsn_t lastLsn, uint64_t expectedDatahash, QueryResults &&wal, QueryResults &&raw)
	: pt_(pt), tm_(std::move(tm)), expectedDatahash_(expectedDatahash), lastLsn_(lastLsn) {
	if (raw.Items().size()) {
		WrSerializer ser;
		tm_.serialize(ser);
...
std::string Snapshot::Dump() {
	...
	for (auto it = Iterator{this, rawData_.Size()}; it != end(); ++it) {
		...
		for (auto &rec : ch.Records()) {
			...
			wrec.Dump(ser, [](string_view) { return std::string("<cjson>"); });
...
SnapshotChunk Snapshot::Iterator::Chunk() const {
	...
	for (auto &itemRef : chunks[idx].items) {
		...
		chunk.records.emplace_back();
		auto &snRec = chunk.records.back();
		if (itemRef.Raw()) {
			pwrec.resize(itemRef.Value().GetCapacity());
			memcpy(pwrec.data(), itemRef.Value().Ptr(), pwrec.size());
			snRec = SnapshotRecord(itemRef.Value().GetLSN(), std::move(pwrec));
		} else {
			ser_.Reset();
