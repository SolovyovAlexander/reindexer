...
void ItemModifier::Modify(IdType itemId, const NsContext &ctx, bool updateWithJson, h_vector<cluster::UpdateRecord, 1> &pendedRepl) {
	if (updateWithJson) {
		modifyCJSON(itemId, pendedRepl, ctx);
	} else {
		modify(itemId, pendedRepl);
	}
}

void ItemModifier::modifyCJSON(IdType itemId, h_vector<cluster::UpdateRecord, 1> &replUpdates, const NsContext &ctx) {
	...
	if (!err.ok()) throw err;
	ns_.doModifyItem(item, ModeUpdate, replUpdates, ctx);
}

void ItemModifier::modify(IdType itemId, h_vector<cluster::UpdateRecord, 1> &pendedRepl) {
	PayloadValue &pv = ns_.items_[itemId];
	Payload pl(ns_.payloadType_, pv);
	pv.Clone(pl.RealSize());
	...
	for (size_t i = 0; i < fieldsToModify_.size(); ++i) {
		modifyField(itemId, fieldsToModify_[i], pl, pendedRepl);
	}
...
void ItemModifier::modifyField(IdType itemId, FieldData &field, Payload &pl, h_vector<cluster::UpdateRecord, 1> &pendedRepl) {
	...
	VariantArray values;
	FunctionExecutor funcExecutor(ns_, pendedRepl);
	ExpressionEvaluator expressionEvaluator(ns_.payloadType_, ns_.tagsMatcher_, funcExecutor);
	if (field.details().isExpression) {
		assert(field.details().values.size() > 0);
