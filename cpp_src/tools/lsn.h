...
struct lsn_t {
	...
	int16_t Server() const { return payload_ / digitCountLSNMult; }
	LSNUnpacked Unpack() const {
		LSNUnpacked unpacked;
		unpacked.server = payload_ / digitCountLSNMult;
		unpacked.counter = payload_ - int64_t(unpacked.server) * digitCountLSNMult;
		return unpacked;
	}
...
// inline static lsn_t operator+(lsn_t lsn, int n) { return lsn_t(lsn.Counter() + n, lsn.Server()); }
